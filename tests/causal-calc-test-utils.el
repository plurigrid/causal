;;; causal-calc-test-utils.el --- Causal Test Utils       -*- lexical-binding: t; -*-
...
(defvar causalt-test-operators-group
  '(("+" . causal-calc--plus)
    ("-" . causal-calc--minus)
    ("*" . causal-calc--times)
    ("/" . causal-calc--divide)
...
(defvar causalt-test-basic-operators-group
  '(("+" . causal-calc--plus)
    ("-" . causal-calc--minus)
    ("*" . causal-calc--times)
    ("/" . causal-calc--divide)))

(defvar causalt-test-navigation-group
  '(("U" . calc-undo)
    ("`" . calc-algebraic-entry)
    ("" . calc-pop)
    ("" . calc-enter)))

(defun causalt-calc-setup ()
  "Causal menu test setup function."
  (calc-create-buffer))

(defun causalt-calc-breakdown (&optional clear dismiss)
  "Causal menu test breakdown function, if CLEAR is non-nil then clear stack."
  (if clear
...
(defun causalt-menu-input-testcase (menu keyseq init value)
...
(defun causalt-menu-assert-testcase (menu keyseq init assert)
...
(defun causalt-run-menu-input-testcases (menu testcases)
...
init - list of arguments to be pushed on the Calc stack before running keyseq
value - value to be compared with top of the stack\n
A testcase is used as input to `causalt-menu-input-testcase'."
  (mapc (lambda (x)
          (causalt-menu-input-testcase menu
                                       (car x)
                                       (nth 1 x)
                                       (nth 2 x)))
        testcases))

(defun causalt-run-menu-assert-testcases (menu testcases)
  "Test runner to exercise MENU with input TESTCASES.
MENU - Transient menu
...
init - list of arguments to be pushed on the Calc stack before running keyseq
assert - assert function that calls `should'\n
A testcase is used as input to `causalt-menu-assert-testcase'."
  (mapc (lambda (x)
          (causalt-menu-assert-testcase menu
                                        (car x)
                                        (nth 1 x)
                                        (nth 2 x)))
        testcases))

(defun causalt-testbench-calc-fn (fn t-args value)
...
VALUE - value to test against the top of the Calc stack"
  (defun causalt-mock-transient-args (prefix)
    "Mock function for transient-args with PREFIX argument."
    t-args)

  (advice-add #'transient-args :override  #'causalt-mock-transient-args)
  (call-interactively fn)
  (should (equal value (calc-top)))
  (advice-remove #'transient-args #'causalt-mock-transient-args))


(defun causalt-testbench-transient-suffix (menu binding cmd value)
...
value to the top of the Calc stack. This value is then tested."
  ...
  (advice-add cmd :override #'causalt-stub)
  (funcall menu)
  (execute-kbd-macro binding)
  (should (equal value (calc-top)))
  (advice-remove cmd #'causalt-stub))

(defun causalt-suffix-testbench-runner (test-vectors menu value-fn)
  "Test runner for suffixes in MENU specified in TEST-VECTORS testing VALUE-FN.
This function executes `causalt-testbench-transient-suffix' for all elements
in TEST-VECTORS.
...
appending \"q\" to the keysequence."
  (mapc (lambda (x)
            (let ((key (car x))
                  (cmd (cdr x)))
              (causalt-testbench-transient-suffix menu
                                                  key
                                                  cmd
...
(defun causalt-macro-callable-symbol (command)
...
(defun causalt-kmacro (command keys)
  "Create `kmacro' instance invoking COMMAND and passing KEYS to drive it.
COMMAND is an interactive function."
  (let ((buf (concat (causalt-macro-callable-symbol command) keys)))
    (kmacro buf)))

(provide 'causal-calc-test-utils)
;;; causal-calc-test-utils.el ends here
