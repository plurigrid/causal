;;; causal-isearch.el --- Transient UI for I-Search -*- lexical-binding: t; -*-
...
;; `causal-isearch-tmenu'. Tune the keybinding to your taste.

;; (require 'causal-isearch) ; optional if using autoloaded menu
;; (keymap-set isearch-mode-map "C-o" #'causal-isearch-tmenu)

;;; Code:
(require 'causal-lib)
(require 'causal-isearch-utils)
(require 'causal-isearch-settings)

;;;###autoload (autoload 'causal-isearch-tmenu "causal-isearch" nil t)
(transient-define-prefix causal-isearch-tmenu ()
  ...
  [["Toggle"
    ("X" "Regexp searching (edit)"
     ...
     :description (lambda () (causal-lib-checkbox-label isearch-regexp
                                                   "Regexp search")))
   ...
   ["Navigation"
    ("p" "Previous" isearch-repeat-backward
     :description (lambda () (causal-isearch-unicode-get :previous))
     :transient t)
    ("n" "Next" isearch-repeat-forward
     :description (lambda () (causal-isearch-unicode-get :next))
     :transient t)
    ("<" "First" isearch-beginning-of-buffer
     :description (lambda () (causal-isearch-unicode-get :first))
     :transient t)
    (">" "Last" isearch-end-of-buffer
     :description (lambda () (causal-isearch-unicode-get :last))
     :transient t)]]

  [:class transient-row
   (causal-lib-quit-one)
   ("," "Settings›" causal-isearch-settings-tmenu)
   ("RET" "Done" isearch-exit)
   (causal-lib-quit-all)])

(provide 'causal-isearch)
;;; causal-isearch.el ends here
