;;; causal-ediff-utils.el --- Causal Eshell Utils -*- lexical-binding: t; -*-
...
(defconst causal-ediff-unicode-db
  '((:previous . '("↑" "Previous"))
    (:next . '("↓" "Next"))
...
(defun causal-ediff-unicode-get (key)
...
plain ASCII-range string."
  (causal-lib-unicode-db-get key causal-ediff-unicode-db))


;; -------------------------------------------------------------------
;;; Format

(defun causal-ediff--buffer-description (ebuf slot &optional extend)
  "Format name of buffer EBUF in SLOT with option to EXTEND."
  (let ((bufname (buffer-name ebuf))
        (fwidth (frame-width)))
    (causal-ediff--display-filename bufname slot extend fwidth)))

(defun causal-ediff--display-filename (filename slot extend fwidth)
...
;; -------------------------------------------------------------------
;;; Ediff Functions
(defun causal-ediff-info ()
  "Open Info for Ediff."
  (interactive) (info "(ediff) Top"))

(defvar causal-ediff--revision-session-p nil
  "If t then `causal-ediff--internal-last-revision' has been called.
This state variable is used to insert added behavior to the advised
function `ediff-janitor'.")

(defvar causal-ediff--installed-p nil
  "If t then Causal Ediff is initialized.")

(defun causal-ediff-revision-from-menu (e)
  "Invoke `causal-ediff-revision' on E with variable `buffer-file-name'."
  (interactive "e")
  (ignore e)
  (causal-ediff-revision))

(defun causal-ediff-revision ()
...
the current buffer."
  ...
  (when (and (bound-and-true-p buffer-file-name)
    ...
    (causal-ediff--internal-last-revision))
...
(defun causal-ediff--internal-last-revision ()
...
This function handles the actual diff behavior called by `ediff-revision'."
  (let ((rev1 "")
        (rev2 ""))
    (setq causal-ediff--revision-session-p t)
    (ediff-load-version-control)
    (funcall
     (intern (format "ediff-%S-internal" ediff-version-control-package))
     rev1 rev2 nil)))

(defun causal-ediff-janitor (ask keep-variants)
...
!!!: This method overrides the original Ediff function."
  (let ((ask (if (and (boundp 'causal-ediff--revision-session-p)
                      causal-ediff--revision-session-p)
                 nil
               ask)))
    (ediff-dispose-of-variant-according-to-user
     ediff-buffer-A 'A ask keep-variants)
    ;; !!!: Test global state variable `causal-ediff--revision-session-p' to
    ...
    (if (and (boundp 'causal-ediff--revision-session-p)
             causal-ediff--revision-session-p)
        (ediff-dispose-of-variant-according-to-user
    ...
    ;; CC Note: Reset global state variable `causal-ediff--revision-session-p'.
    (if (and (boundp 'causal-ediff--revision-session-p)
             causal-ediff--revision-session-p)
        (setq causal-ediff--revision-session-p nil))))


(defun causal-ediff--stash-window-configuration-for-ediff ()
...
(defun causal-ediff--restore-window-configuration-for-ediff ()
...
(defun causal-ediff-copy-AB-to-C ()
...
(defun causal-ediff-copy-BA-to-C ()
...
(provide 'causal-ediff-utils)
;;; causal-ediff-utils.el ends here
