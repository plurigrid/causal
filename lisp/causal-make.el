;;; causal-make.el --- Transient UI for Make -*- lexical-binding: t; -*-
...
;; In your initialization file, bind the Transient `causal-make-tmenu' to your
;; key binding of preference. Two suggested bindings are 'M-m' and 'C-c m'.

;; (require 'causal-make) ; optional if using autoloaded menu
...
;;;###autoload (autoload 'causal-make-tmenu "causal-make" nil t)
(transient-define-prefix causal-make-tmenu ()
  "Main menu for Causal Make.

This menu provides a user interface for the commands in `make-mode'."

  ["Causal Make"
   :description (lambda () (format "Causal Make (%s)" (causal-make-mode-label major-mode)))
   ["Edit"
    ...
    ("a" "Automatic Variables›" causal-make-automatic-variables-tmenu)]
   ...
   ["Misc"
    ...
    ("t" "Makefile Type›" causal-make-mode-select-tmenu :transient t)
    ("." "Identify Auto Var" causal-make-identify-autovar-region
   ...
   ["Navigate"
    ...
    ("p" "Previous" makefile-previous-dependency
     :description (lambda ()
                    (format "%s target" (causal-make-unicode-get :previous)))
     :transient t)
    ("n" "Next" makefile-next-dependency
     :description (lambda ()
                    (format "%s target"(causal-make-unicode-get :next)))
     :transient t)]]

  [:class transient-row
   (causal-lib-quit-one)
   ("," "Settings" causal-make-settings-tmenu)
   ("I" "ⓘ Make" (lambda () (interactive) (info "(make) Top")))
   ("RET" "Done" transient-quit-all)
   (causal-lib-quit-all)])

(provide 'causal-make)
;;; causal-make.el ends here
