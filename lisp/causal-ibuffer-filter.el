;;; causal-ibuffer-filter.el --- Causal IBuffer Filter -*- lexical-binding: t; -*-
...
;;; Code:
(defvar causal-ibuffer--current-collection-name nil
  "Internal variable for current filter group collection name.")

(defun causal-ibuffer-switch-to-saved-filter-groups (name)
...
The value from `ibuffer-saved-filter-groups' is used."
  ...
  (setq causal-ibuffer--current-collection-name name)
  (setq ibuffer-filter-groups (cdr (assoc name ibuffer-saved-filter-groups))
	ibuffer-hidden-filter-groups nil)
  (ibuffer-update nil t))

(defun causal-ibuffer--ibuffer-update (arg &optional silent)
  "Advising function after `ibuffer-update' is called with ARG and SILENT."
  (ignore arg)
  (ignore silent)
  (if (not ibuffer-filter-groups)
      (setq causal-ibuffer--current-collection-name nil)))

(advice-add #'ibuffer-update :after #'causal-ibuffer--ibuffer-update)

;; Transients
;;;###autoload (autoload 'causal-ibuffer-filter-tmenu "causal-ibuffer-filter" nil t)
(transient-define-prefix causal-ibuffer-filter-tmenu ()
  "Causal IBuffer filter menu."
  :refresh-suffixes t
  ["IBuffer: Filter"
   ["Add"
    :pad-keys t
  ...
  ["Filter Group"
   ...
   ["Collection"
    :description (lambda ()
                   (format "Collection%s"
                           (if causal-ibuffer--current-collection-name
                               (format
                                " (%s)"
                                causal-ibuffer--current-collection-name)
                             "")))
    ("R" "Switch to…" causal-ibuffer-switch-to-saved-filter-groups)
...
(provide 'causal-ibuffer-filter)
;;; causal-ibuffer-filter.el ends here
