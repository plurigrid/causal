;;; causal-dired.el --- Transient UI for Dired -*- lexical-binding: t; -*-
...
;; Causal Dired is an opinionated Transient-based user interface for Emacs Dired.

;; INSTALLATION
;; (require 'causal-dired) ; optional if using autoloaded menu
;; (keymap-set dired-mode-map "C-o" #'causal-dired-tmenu)
;; (keymap-set dired-mode-map "s" #'causal-dired-sort-by-tmenu) ; optional
;; (keymap-set dired-mode-map "/" #'causal-dired-search-replace-tmenu) ; optional
...
;;; Menus
;;;###autoload (autoload 'causal-dired-tmenu "causal-dired" nil t)
(transient-define-prefix causal-dired-tmenu ()
  ...
  [["File"
    ...
    ("c" "Change›" causal-dired-change-tmenu :transient nil)
   ...
   ["Directory"
    ("s" "Sort By›" causal-dired-sort-by-tmenu
     :if causal-dired-show-sort-by-tmenu-p
     :transient t)
    ("h" "Hide Details" dired-hide-details-mode
     :description
     (lambda ()
       (causal-lib-checkbox-label dired-hide-details-mode "Hide Details"))
     :if-not causal-dired-lisp-dired-buffer-p
     :transient t)
    ("O" "Omit Mode" dired-omit-mode
     :description
     (lambda () (causal-lib-checkbox-label dired-omit-mode "Omit Mode"))
     :transient t)
    ...
    ("f" "Filter by name…" causal-dired-find-dired-regexp)
   ...
   ["Mark"
    ...
    ("r" "Regexp›" causal-dired-regexp-tmenu :transient nil)
    ("#" "Utils›" causal-dired-utils-tmenu :transient nil)
    ("/" "Search & Replace›" causal-dired-search-replace-tmenu :transient nil)]

   ["Navigation"
    :pad-keys t
    ("^" ".." dired-up-directory
     :description (lambda ()
                    (format ".. %s" (causal-dired-unicode-get :directory)))
     :transient t)

    ("p" " ↑ 📄" dired-previous-line
     :description (lambda ()
                    (format "%s %s"
                            (causal-dired-format-arrow
                             (causal-dired-unicode-get :up-arrow)
                             causal-lib-use-unicode)
                            (causal-dired-unicode-get :file)))
     :transient t)
    ...
    ("n" " ↓ 📄" dired-next-line
     :description (lambda ()
                    (format "%s %s"
                            (causal-dired-format-arrow
                             (causal-dired-unicode-get :down-arrow)
                             causal-lib-use-unicode)
                            (causal-dired-unicode-get :file)))
     :transient t)
     ...
     :description (lambda ()
                    (format "%s %s"
                            (causal-dired-format-arrow
                             (causal-dired-unicode-get :up-arrow)
                             causal-lib-use-unicode)
                            (causal-dired-unicode-get :directory)))
     :transient t)
     ...
     :description (lambda ()
                    (format "%s %s"
                            (causal-dired-format-arrow
                             (causal-dired-unicode-get :up-arrow)
                             causal-lib-use-unicode)
                            (causal-dired-unicode-get :subdir)))
     :transient t)
    ("]" " ↓ 🗂️" dired-next-subdir
     :if-not causal-dired-lisp-dired-buffer-p
     :description (lambda ()
  ...
  [:class transient-row
          ...
          ("," "Settings›" causal-dired-settings-tmenu :transient nil)
          ("q" "Quit Dired" quit-window)])

(transient-define-prefix causal-dired-regexp-tmenu ()
...
(transient-define-prefix causal-dired-change-tmenu ()
  ["Change"
...
(defun causal-dired-show-sort-by-tmenu-p ()
  "Predicate to show `causal-dired-sort-by-tmenu'."
  (and (equal dired-use-ls-dired t)
       (not (causal-dired-lisp-dired-buffer-p))))

(defun causal-dired-find-dired-regexp (REGEXP)
...
;;; Labels
(defun causal-dired-format-arrow (buf typeset)
  "If TYPESET is non-nil, then format BUF string to have space."
  (if typeset
      (format " %s" buf)
    buf))

(provide 'causal-dired)
;;; causal-dired.el ends here
