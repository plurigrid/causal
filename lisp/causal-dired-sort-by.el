...
;; causal-dired-sort-by is a set of functions that provide richer sorting
...
;; (define-key dired-mode-map (kbd "s") 'causal-dired-sort-by-tmenu)
;;
;; The keymap `causal-dired-sort-menu' can be used in a context menu
;; (see `context-menu-mode') or some other main menu.
;;
;; Typically this is done with code looking like this:

;; (defun causal-dired-context-menu-addons (menu click)
;;   "Customize context MENU with CLICK event."
;;   (easy-menu-add-item menu nil causal-dired-sort-menu)
;;   menu)
;; (add-hook 'context-menu-functions #'causal-dired-context-menu-addons)
...
;;;###autoload (autoload 'causal-dired-sort-by-tmenu "causal-dired-sort-by" nil t)
(transient-define-prefix causal-dired-sort-by-tmenu ()
  "Transient menu to sort Dired buffer by different criteria.

This function requires GNU ls from coreutils installed."
  :value causal-dired-listing-switches
  :man-page "ls"
  [["Options"
   ...
   ["Sort By"
    ("n" "Name" causal-dired--sort-by-name :transient nil)
    ("k" "Kind" causal-dired--sort-by-kind :transient nil)
    ("l" "Date Last Opened" causal-dired--sort-by-date-last-opened
     :transient nil)
    ("a" "Date Added" causal-dired--sort-by-date-added :transient nil)
    ("m" "Date Modified" causal-dired--sort-by-date-modified :transient nil)
    ("M" "Date Metadata Changed" causal-dired--sort-by-date-metadata-changed
     :transient nil)

    ("s" "Size" causal-dired--sort-by-size :transient nil)]]

  [:class transient-row
          (causal-lib-quit-one)
          (causal-lib-quit-all)])

(defun causal-dired--sort-by-name ()
  "Sort directory by name.

This command passes the appropriate arguments to GNU ls to sort
the directory by name."
  (interactive)
  (causal-dired--sort-by
   :name
   (transient-args transient-current-command)))

(defun causal-dired--sort-by-kind ()
...
(defun causal-dired--sort-by-date-last-opened ()
...
(defun causal-dired--sort-by-date-added ()
...
(defun causal-dired--sort-by-date-modified ()
...
(defun causal-dired--sort-by-date-metadata-changed ()
...
(defun causal-dired--sort-by-version ()
...
(defun causal-dired--sort-by-size ()
...
(defun causal-dired--sort-by (criteria &optional prefix-args)
...
of `causal-dired-listing-switches'. Otherwise this is typically populated by the
Transient menu `causal-dired-sort-by-tmenu'.
...
See the man page `ls(1)' for details."
  ...
  (let ((arg-list (list "-l")))
    (if prefix-args
      ...
      (nconc arg-list causal-dired-listing-switches))
    (cond
     ((eq criteria :name)
    ...
    (causal-dired--update-mode-name criteria (member "--reverse" arg-list))))

(defun causal-dired--update-mode-name (criteria reverse)
  "Update Dired `mode-name' given CRITERIA and REVERSE.

Update the value of `mode-name' as part of the sort operation
done by `causal-dired-sort-by'.
...
- REVERSE: if non-nil then buffer sorted with the --reverse argument"
  ...
  (setq mode-name (causal-dired--mode-name-from-sort criteria reverse))
  (force-mode-line-update))

(defun causal-dired--mode-name-from-sort (criteria reverse)
...
(easy-menu-define causal-dired-sort-menu nil
  "Keymap for Dired sort by menu."
  '("Sort By"
    :visible (and (derived-mode-p 'dired-mode) (not dired-sort-inhibit))
    ["Name"
     (lambda () (interactive) (causal-dired--sort-by :name))
     :help "Sort by name"]
    ["Kind"
     (lambda () (interactive) (causal-dired--sort-by :kind))
     :help "Sort by kind"]
    ["Date Last Opened"
     (lambda () (interactive) (causal-dired--sort-by :date-last-opened))
     :help "Sort by date last opened"]
    ["Date Added"
     (lambda () (interactive) (causal-dired--sort-by :date-added))
     :help "Sort by date added"]
    ["Date Modified"
     (lambda () (interactive) (causal-dired--sort-by :date-modified))
     :help "Sort by date modified"]
    ["Date Metadata Changed"
     (lambda () (interactive) (causal-dired--sort-by :date-metadata-changed))
     :help "Sort by date metadata changed"]
    ["Version"
     (lambda () (interactive) (causal-dired--sort-by :version))
     :help "Sort by version"]
    ["Size"
     (lambda () (interactive) (causal-dired--sort-by :size))
     :help "Sort by size"]
    "--"
    ("Reverse Sort By"
     ["Name"
      (lambda () (interactive) (causal-dired--sort-by
                                :name
...
(provide 'causal-dired-sort-by)
;;; causal-dired-sort-by.el ends here
