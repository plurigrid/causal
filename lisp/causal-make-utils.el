;;; causal-make-utils.el --- Causal Make Utils -*- lexical-binding: t; -*-
...
(defconst causal-make-unicode-db
  '((:previous . '("↑" "Previous"))
    (:next . '("↓" "Next")))

  "Unicode symbol DB to use for Make Transient menus.")

(defun causal-make-unicode-get (key)
...
plain ASCII-range string."
  (causal-lib-unicode-db-get key causal-make-unicode-db))


(defconst causal-make--make-mode-label-map
...
(defun causal-make-mode-label (mode)
  "Label for make MODE."
  (map-elt causal-make--make-mode-label-map mode))

(transient-define-prefix causal-make-mode-select-tmenu ()
  ...
  ["Select Make Mode"
   :description (lambda ()
                  (format
                   "Select Make Mode (%s)"
                   (causal-make-mode-label major-mode)))
...
(defconst causal-make--autovar-description-map
  '(("$@" . "The target name.")
...
(provide 'causal-make-utils)
;;; causal-make-utils.el ends here
