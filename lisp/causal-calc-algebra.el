;;; causal-calc-algebra.el --- Causal wrapped algebra functions  -*- lexical-binding: t; -*-
...
(defun causal-calc--find-maximum ()
...
(defun causal-calc--alg-evaluate ()
...
(defun causal-calc--alt-summation ()
...
(defun causal-calc--clear-selections ()
...
(defun causal-calc--commute-left ()
...
(defun causal-calc--commute-right ()
...
(defun causal-calc--copy-selection ()
...
(defun causal-calc--del-selection ()
...
(defun causal-calc--edit-selection ()
...
(defun causal-calc--enter-selection ()
...
(defun causal-calc--expand-formula ()
...
(defun causal-calc--greater-equal ()
...
(defun causal-calc--greater-than ()
  "Compare top two elements in the stack if they are > to each other.
\nThis command tests a > b and places the result in the top of stack.
\nStack Arguments:
2: a
...
(defun causal-calc--why ()
  "Provide explanation for why no root was found.
\nIn the event that `causal-calc--find-root' is unable to find a
...
(defun causal-calc--map-equation ()
...
(defun causal-calc--normalize-rat ()
...
(defun causal-calc--not-equal-to ()
...
(defun causal-calc--num-integral ()
...
(defun causal-calc--poly-div-rem ()
...
(defun causal-calc--remove-equal ()
...
(defun causal-calc--sel-distribute ()
...
(defun causal-calc--sel-evaluate ()
...
(defun causal-calc--sel-jump-equals ()
...
(defun causal-calc--sel-merge ()
  "Merge selected term.
\nThis can be viewed as the complement to
`causal-calc--sel-distribute'.
...
(defun causal-calc--select-previous ()
...
(provide 'causal-calc-algebra)
;;; causal-calc-algebra.el ends here
