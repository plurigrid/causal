;;; causal-calc-fileio.el --- Causal File I/O Routines    -*- lexical-binding: t; -*-
...
(defun causal-calc-datafile-to-matrix (filename)
...
;;(kill-new (pp (causal-calc-datafile-to-matrix "~/fred.dat")))

(defun causal-calc-matrix-to-calc-vector (matrix)
  "Convert matrix into a Calc vector MATRIX.

This function converts the output of `causal-calc-datafile-to-matrix'
to a Calc matrix."
  (let ((templist (list)))
    (mapc (lambda (x)
...
(defun causal-calc-read-curvefit-data (filename)
...
- `causal-calc--curve-fit'"
  ...
  (calc-transpose
   (calc-push
    (causal-calc-matrix-to-calc-vector
     (causal-calc-datafile-to-matrix filename)))))

(defun causal-calc-read-plot-data (filename)
...
data can be plotted using either `causal-calc--graph-add' or
`causal-calc--graph-add-3d'."
...
(provide 'causal-calc-fileio)
;;; causal-calc-fileio.el ends here
