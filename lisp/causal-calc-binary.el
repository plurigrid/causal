;;; causal-calc-binary.el --- Causal Binary Menu          -*- lexical-binding: t; -*-
...
(transient-define-prefix causal-calc-binary-tmenu ()
  "Causal binary functions menu."
  ["Binary Functions"
   ["Operators"
    ("&" "and" causal-calc--and :transient t)
    ("|" "or" causal-calc--or :transient t)
    ("^" "xor" causal-calc--xor :transient t)
    ("d" "diff" causal-calc--diff :transient t)
    ("!" "not" causal-calc--not :transient t)]
   ["Shift"
    :pad-keys t
    ("l" "binary left" causal-calc--lshift-binary :transient t)
    ("r" "binary right" causal-calc--rshift-binary :transient t)
    ("M-l" "arithmetic left" causal-calc--lshift-arith :transient t)
    ("M-r" "arithmetic right" causal-calc--rshift-arith :transient t)
    ("C-r" "rotate binary" causal-calc--rotate-binary :transient t)]
   ...
   ["Utils"
    ...
    ("z" "Leading Zeroes" calc-leading-zeros
     :description (lambda ()
                    (causal-lib-checkbox-label calc-leading-zeros "Leading Zeroes"))
     :transient t)
    ("," "Thousands Separator…" calc-group-char
     :description (lambda ()
                    (format "Set Thousands Separator (now %s)…" calc-group-char))
     :transient t)
    ("w" "Set Word Size…" causal-calc--word-size :transient t)
    ("u" "Unpack Bits" causal-calc--unpack-bits :transient t)
    ("p" "Pack Bits" causal-calc--pack-bits :transient t)]]
...
(defun causal-calc--lshift-binary ()
...
(defun causal-calc--rshift-binary ()
...
(defun causal-calc--lshift-arith ()
...
(defun causal-calc--rshift-arith ()
...
(defun causal-calc--rotate-binary ()
...
(provide 'causal-calc-binary)
;;; causal-calc-binary.el ends here
