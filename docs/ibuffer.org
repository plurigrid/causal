* IBuffer
#+CINDEX: IBuffer
#+VINDEX: causal-ibuffer

Causal IBuffer (library: ~causal-ibuffer~) provides a user interface to Emacs IBuffer ([[info:emacs#Buffer Menus][emacs#Buffer Menus]]), a mode designed for managing buffers.
...
[[file:images/causal-ibuffer-screenshot.png]]
...
In your initialization file, bind the Transinent ~causal-ibuffer-tmenu~ to your key binding of preference. 

#+begin_src elisp :lexical no
  (keymap-set ibuffer-mode-map "C-o" #'causal-ibuffer-tmenu)
#+end_src

Like with Causal Dired, it is convenient to have the menus for filtering and sorting bound as well. Listed below shows an example of binding ~causal-ibuffer-filter-tmenu~ and ~causal-ibuffer-sortby-tmenu~ to ~F~ and ~s~ respectively.
...
Use these keybindings to configure IBuffer to be consistent with keybindings used by Causal IBuffer.
...
While not necessary for Causal IBuffer, enabling ~hl-line-mode~ and binding mouse clicks in IBuffer adds to a more comfortable IBuffer experience. Also, adding ~ibuffer-auto-mode~ to ~ibuffer-mode-hook~ will enable auto-updating. 
...
#+VINDEX: causal-ibuffer-tmenu

[[file:images/causal-ibuffer-screenshot.png]]


The main menu of Causal IBuffer (~causal-ibuffer-tmenu~) is organized into the following sections:
...
[[file:images/causal-ibuffer-main-screenshot.png]]
...
As with other Causal user interfaces, the ability to jump to a bookmark is available.

# TODO: Need to add operations, mark, mark-regexp tmenus

*** IBuffer Filtering
#+VINDEX: causal-ibuffer-filter-tmenu

[[file:images/causal-ibuffer-filter-screenshot.png]]
...
1. *Filter rule*
   ...
   - filter by buffers visiting files
    ...
    Causal IBuffer makes the design decision to *not* enumerate the above in a menu, delegating the work of filter selection to the command  ~ibuffer-filter-chosen-by-completion~.
...
Out of the box, it is best to think of the IBuffer commands for editing buffer filters as a kit of parts and an arguably incomplete one at that. The Causal IBuffer filter menu (=causal-ibuffer-filter-tmenu=) is my attempt to build a comprehensible filter editor UI from this kit. Whether it succeeds in being comprehensible is left to user feedback.


*** IBuffer Sorting
#+VINDEX: causal-ibuffer-sortby-tmenu

[[file:images/causal-ibuffer-sortby-screenshot.png]]

The buffer list can be sorted using different criteria using the menu ~causal-ibuffer-sortby-tmenu~.

Sort ordering can be reversed via the /Invert/ command.

*** IBuffer Unicode Symbol Support
By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Causal IBuffer will use Unicode symbols as appropriate in its menus.
