* Info
#+CINDEX: Info
#+VINDEX: causal-info-tmenu

Causal Info is a user interface for the Emacs Info Reader. Its top level library is ~causal-info~.

[[file:images/causal-info-screenshot.png]]
...
#+CINDEX: Info Install
The main menu for Causal Info is ~causal-info-tmenu~. Bind this menu in the keymap ~Info-mode-map~ as follows in your initialization file.

#+begin_src elisp :lexical no
  (keymap-set Info-mode-map "C-o" #'causal-info-tmenu)
#+end_src

While not required, adding this configuration to your Emacs initialization file will synchronize keybindings between Causal Info and the Info reader. A nice visual improvement is to use ~hl-line-mode~ to highlight the line where the cursor is at. Enabling ~scroll-lock-mode~ will enable scrolling the buffer for content that is larger than its window size with the navigation keys.
...
#+begin_src elisp :lexical no
  ...
  (keymap-set Info-mode-map "p" #'causal-info-browse-backward-paragraph)
  (keymap-set Info-mode-map "n" #'causal-info-browse-forward-paragraph)
...
Invoke ~M-x info~ to launch the Info reader. Move the point inside the Info window and invoke {{{kbd(C-o)}}} (or a binding of your choosing) to launch the Causal Info menu.

The main menu for Causal Info is organized into the following sections:
...
By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Causal Info will use Unicode symbols as appropriate in its menus. 
