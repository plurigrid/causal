...
#+CINDEX: BibTeX
#+CINDEX: bibtex
#+VINDEX: causal-bibtex

Causal BibTeX (library: ~causal-bibtex~) is an opinionated user interface for BibTeX mode ([[info:emacs#TeX Mode][info "(emacs) TeX Mode"]]), a major mode for working with BibTeX files ([[https://bibtex.eu][BibTeX Guide]]).

A BibTeX file is typically named using the suffix ~.bib~ to identify its file type.

To appreciate the benefits of Causal BibTeX, it helps to understand what BibTex is and what BibTeX mode offers.
...
Causal BibTeX organizes and augments BibTeX mode commands into a keyboard-driven menu.

The screenshot below shows the menu for Causal BibTeX.

[[file:images/causal-bibtex-screenshot.png]]
...
In your initialization file, bind the Transient ~causal-bibtex-tmenu~ to your key binding of preference in ~bibtex-mode-map~. The binding {{{kbd(M-m)}}} is suggested so as to not conflict with {{{kbd(C-o)}}} that is bound to ~causal-editkit-main-tmenu~. ([[#editkit-install][EditKit Install]])

#+begin_src elisp :lexical no
  (keymap-set bibtex-mode-map "M-m" #'causal-bibtex-tmenu)
#+end_src

~causal-bibtex-tmenu~ is opinionated in making editing and navigation commands emulate a form-based interface ([[https://simple.wikipedia.org/wiki/Form-based_interface#:~:text=A%20form%2Dbased%20interface%20is,the%20fields%20that%20accept%20it.][form-based interface]]) in a ~bibtex-mode~ window. The following keybindings are recommended to support consistent behavior between ~bibtex-mode-map~ and ~causal-bibtex-tmenu~.
...
#+begin_src elisp :lexical no
  ...
  (keymap-set bibtex-mode-map "C-c C-c" #'causal-bibtex-fill-and-clean)
...
** BibTeX Usage
#+CINDEX: BibTeX Usage
#+VINDEX: causal-bibtex-tmenu

[[file:images/causal-bibtex-screenshot.png]]

The main menu for Causal BibTeX (~causal-bibtex-tmenu~) is organized into the following sections:
...
Causal BibTeX re-purposes conventional Emacs navigation bindings so that instead of navigating a text edit buffer, it instead navigates BibTeX structures (entry, field).
...
| {{{kbd(C-a)}}} | causal-bibtex-beginning-of-field                  |
| {{{kbd(C-e)}}} | causal-bibtex-end-of-field                        |
...
In Causal BibTeX, CRUD (CReate, Update, Delete) operations for entry and field structures are intended to be achieved through their respective commands. This contrasts with manual editing of a buffer to achieve the same end, which is prone to errors.
...
#+VINDEX: causal-bibtex-settings-tmenu

[[file:images/causal-bibtex-settings-screenshot.png]]


The menu ~causal-bibtex-settings-tmenu~ offers access to commonly configured BibTeX mode settings.
...
[[file:images/causal-bibtex-clean-entry-hook.png]]
*** BibTeX Mode Unicode Symbol Support

By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Causal BibTeX will use Unicode symbols as appropriate in its menus.

[[file:images/causal-bibtex-unicode-screenshot.png]]
