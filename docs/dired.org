...
#+CINDEX: Dired
#+VINDEX: causal-dired
Causal Dired (library: ~causal-dired~) provides a user interface for Dired ([[info:emacs#Dired]]), the Emacs file manager. Its top-level library is ~causal-dired~.

[[file:images/causal-dired-screenshot.png]]

** Dired Requirements
Causal Dired requires that the ~ls~ utility from GNU coreutils ≥ 8.32 be installed.

The following links provide guidance for installing GNU coreutils on different platforms.

*** macOS
Note that the default packaged ~ls~ on macOS is BSD-flavored which is not supported by Causal Dired. Users wishing to use Causal Dired on macOS are recommended to install GNU coreutils and configure their Emacs to point to its version of ~ls~ accordingly.
...
The main menu for Dired is ~causal-dired-tmenu~. Bind this menu in the keymap ~dired-mode-map~ as follows in your initialization file.

#+begin_src elisp :lexical no
  (keymap-set dired-mode-map "C-o" #'causal-dired-tmenu)
#+end_src

In addition, it is convenient to have both the sort-by (~causal-dired-sort-by-tmenu~) and search & replace (~causal-dired-search-replace-tmenu~) menus bound. Listed below is an example of binding the sort-by and search & replace menus to ~s~ and ~/~ respectively.

#+begin_src elisp :lexical no
  (keymap-set dired-mode-map "s" #'causal-dired-sort-by-tmenu)
  (keymap-set dired-mode-map "/" #'causal-dired-search-replace-tmenu)
#+end_src

Included is a standard keymap for Dired sorting commands (~causal-dired-sort-menu~) which can be included in a context menu for a mouse-driven workflow. An example of this is shown below:
...
As Dired has been around for a long time, the different ways of configuring it are myriad. Described below is a configuration used by the author that is consistent with the bindings used in Causal Dired.
...
The Causal Dired main menu offers “{{{kbd(\,)}}} Settings›” to customize a set of commonly used Dired variables. 

[[file:images/causal-dired-settings-screenshot.png]]
...
** Dired Usage
#+CINDEX: Dired Usage
#+VINDEX: causal-dired-tmenu

[[file:images/causal-dired-screenshot.png]]

Invoke ~M-x dired~ to launch Dired. When the point is in the Dired window, invoke {{{kbd(C-o)}}} (or a binding of your choosing) to launch the Causal Dired menu (~causal-dired-tmenu~).

Causal Dired organizes its main menu into the following sections:
...
#+VINDEX: causal-dired-sort-by-tmenu

Causal Dired offers enhanced sorting capabilities through GNU ~ls~. Use this to sort your Dired buffer to preference. Filter dot files (.*) by disabling the ~--all~ option is available here.

[[file:images/causal-dired-sort-by-screenshot.png]]
...
#+VINDEX: causal-dired-search-replace-tmenu

Search and replace in multiple marked files using the “{{{kbd(/)}}} Search & Replace›” menu  (~causal-dired-search-replace-tmenu~) in ~causal-dired-tmenu~. This will raise a menu of Dired commands that will work on marked files.

[[file:images/causal-dired-search-replace.png]]
...
#+VINDEX: causal-dired-utils-tmenu

Causal Dired organizes a number of Dired commands that work on a set of marked files. These commands are presented in the “{{{kbd(#)}}} Utils›” menu (~causal-dired-utils-tmenu~) in ~causal-dired-tmenu~.

[[file:images/causal-dired-utils-screenshot.png]]

*** Dired Link
#+VINDEX: causal-dired-link-tmenu

Create symbolic and hard links via the “{{{kbd(l)}}} Link›” menu (~causal-dired-link-tmenu~) in ~causal-dired-tmenu~. Both absolute and relative symbolic links (symlinks) are supported.

[[file:images/causal-dired-link.png]]


*** Dired Elisp
#+VINDEX: causal-dired-elisp-tmenu

Support for running bulk commands for Elisp development are provided by the menu ~causal-dired-elisp-tmenu~.
...
By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Causal Dired will use Unicode symbols as appropriate in its menus. An example is shown below.

[[file:images/causal-dired-screenshot-unicode.png]]
