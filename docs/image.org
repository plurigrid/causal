* Image
#+CINDEX: Image
#+VINDEX: causal-image-tmenu

Causal Image is a user interface for Image Mode ([[info:emacs#Image Mode]]). Its top level library is ~causal-image~.
...
[[file:images/causal-image-main-screenshot.png]]
...
The main menu for Causal Image is ~causal-image-tmenu~. Bind this menu in the keymap ~image-mode-map~ as follows in your initialization file.

#+begin_src elisp :lexical no
  (keymap-set image-mode-map "C-o" #'causal-image-tmenu)
#+end_src
...
The main menu for Causal Image (~causal-image-tmenu~) is organized into the following sections:
...
*** Image Resize
#+CINDEX: Image Resize
#+VINDEX: causal-image-resize-tmenu

If ImageMagick (version 6 or 7) is installed, Causal Image can resize an image using it. The Transient ~causal-image-resize-tmenu~ is a streamlined interface to the ImageMagick ~-resize~ function.

Note that if the image file has been modified, the resize interface will be disabled. Save the image file before resizing.

[[file:images/causal-image-resize-screenshot.png]]
...
*** Image Settings
#+VINDEX: causal-image-settings-tmenu

The menu ~causal-image-settings-tmenu~ provides access to different ~image-mode~ settings.

# TODO: Insert screenshot

*** Image Unicode Symbol Support

By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Causal Image will use Unicode symbols as appropriate in its menus.

[[file:images/causal-image-main-unicode-screenshot.png]]

For more info on using Unicode symbols, please refer to [[#ux-conventions][UX Conventions]].

*** Image Mode Command Naming

Causal Image makes a number of opinionated changes to the naming of commands provided by ~image-mode~. 

The table below shows the mapping between names used by Causal to commands provided by ~image-mode~.

| Causal Name           | Image Mode Name                   | Notes                                                                                            |
|-----------------------+-----------------------------------+--------------------------------------------------------------------------------------------------|
