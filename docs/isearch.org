* I-Search
#+CINDEX: isearch
#+CINDEX: I-Search
#+VINDEX: causal-isearch-tmenu

Causal I-Search is a user interface for Emacs Incremental Search ([[info:emacs#Incremental Search][emacs#Incremental Search)]]. Its top level library is ~causal-isearch~.

[[file:images/causal-isearch-tmenu.png]]

** I-Search Install
:PROPERTIES:
:CUSTOM_ID: i-search-install
:END:
#+CINDEX: I-Search Install

The main menu for Causal I-Search is ~causal-isearch-tmenu~. Bind this menu to {{{kbd(C-o)}}} (or your binding of preference) in the keymap ~isearch-mode-map~ as follows in your initialization file.

#+begin_src elisp :lexical yes
  (keymap-set isearch-mode-map "C-o" #'causal-isearch-tmenu)
#+end_src


** I-Search Usage
#+CINDEX: I-Search Usage

[[file:images/causal-isearch-tmenu.png]]

The main menu for Causal I-Search is organized into the following sections:

- Edit Search String :: Commands to edit the search string. The type/extent of the string (word, symbol, line, thing) can be specified here.

- Replace :: Invoke ~query-replace~ or ~query-replace-regexp~ on matched strings.

- Toggle :: Commands to configure the type of search. Toggling any menu item in this section will dismiss the Transient menu and return you back to the I-Search minibuffer prompt.

- Misc :: Miscellaneous commands. From here the search string can be fed into ~occur~ or be highlighted.

- Navigation :: Navigation commands for matched strings.

When in search mode (typically via the keybinding {{{kbd(C-s)}}} or {{{kbd(C-r)}}}), pressing the keybinding {{{kbd(C-o)}}} (or binding of your preference) will raise the Transient menu ~causal-isearch-tmenu~. Once raised, only the /I-Search/ commands in the *Toggle*, *Replace*, and *Misc* sections will automatically dismiss the menu when selected. All other /I-Search/ commands will /not/ dismiss the menu.

Use {{{kbd(C-g)}}} to dismiss this Transient menu.


#+TEXINFO: @subheading I-Search Settings
#+VINDEX: causal-isearch-settings-tmenu

The menu ~causal-isearch-settings-tmenu~ provides access to different I-Search settings.

[[file:images/causal-isearch-settings-tmenu.png]]


#+TEXINFO: @subheading I-Search Unicode Symbol Support

By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Causal I-Search will use Unicode symbols as appropriate in its menus.

[[file:images/causal-isearch-unicode.png]]

For more info on using Unicode symbols, please refer to [[#ux-conventions][UX Conventions]].



