* RE-Builder
#+CINDEX: RE-Builder
#+VINDEX: causal-re-builder

Causal RE-Builder (library: ~causal-re-builder~) is a user interface for RE-Builder.

[[file:images/causal-re-builder-screenshot.png]]
...
The main menu for Causal RE-Builder is ~causal-re-builder-tmenu~. Bind this menu to your preference in the keymaps ~reb-mode-map~ and ~reb-lisp-mode-map~ as follows in your initialization file.
...
#+VINDEX: causal-re-builder-tmenu

[[file:images/causal-re-builder-screenshot.png]]

When the command ~re-builder~ is invoked, a buffer named "✳︎RE-Builder✳︎" is created. Activate Causal RE-Builder with the binding {{{kbd(C-o}}} (or one of your preference). 
...
- “{{{kbd(g)}}} Interactive grep” will copy the regexp so that it can be used with command that take a GNU grep regex argument.
  ...
  - ❗️At current this is an experimental feature. The regexp exported from RE-Builder may not work. If so please report an [[https://github.com/kickingvegas/causal-re-builder/issues][issue]] describing the desired regexp and the target text.
  - This can only be used when the regexp syntax is set to ~string~.    
...
#+VINDEX: causal-re-builder-settings-tmenu

The menu ~causal-re-builder-settings-tmenu~ provides access to different RE-Builder settings.

# TODO: Insert screenshot

#+TEXINFO: @subsubheading RE-Builder Unicode Symbol Support
By enabling “{{{kbd(u)}}} Use Unicode Symbols” from the Settings menu, Causal RE-Builder will use Unicode symbols as appropriate in its menus.
