...
Causal Calc (library: ~causal-calc~) is a user interface for Emacs Calc ([[info:calc#Top]]).
...
To install Causal Calc, add the following lines to your Emacs initialization file with your binding of preference.
...
** Calc Usage
#+CINDEX: Calc Usage
#+VINDEX: causal-calc-tmenu

[[file:images/causal-calc-tmenu.png]]

To launch Calc, invoke  ~M-x calc~. When the point is in the Calc window, invoke {{{kbd(C-o)}}} (or a binding of your choosing) to launch the Causal Calc interface.
...
Causal Calc organizes its main menu into the following sections:
